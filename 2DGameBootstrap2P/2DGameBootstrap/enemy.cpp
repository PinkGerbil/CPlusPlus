...
{
	spriteBatch->drawSprite(m_EnemyTex, m_enemyPos->x, m_enemyPos->y, 50.0f, 50.0f, 9.4f);
...
void enemy::Update(float deltaTime)
{
	...
	if (timer2 > 0.5f) {
		...
		if (m_inActiveEnemyBullets.size() > 0) {
				...
				newenemyBullet->bullEnemyPosX = m_enemyPos->x + 10;
				newenemyBullet->bullEnemyPosY = m_enemyPos->y - 4;
